			...
			<div id="page_control">
				<%
					if(count > 0){
						...
						//전체 페이지수 구하기   글 20개  한페이지당 보여줄 글개수 10개  => 2개의 페이지
						//                 글 25개  한페이지당 보여줄 글개수 10개  => 3개의 페이지
						//조건삼항연산자 사용   ->   조건식  ?  참일경우실행문 : 거짓일경우실행문
						//전체페이지수  = 전체글개수 / 한페지당 보여줄 글개수  + (전체글수를 한페이지에 보여줄 글수로 나눈 나머지값)		
						...
						//((현재 클릭한 페이지번호 / 한블럭에 보여줄 페이지 개수 )
						//-(현재 클릭한 페이지번호를  한블럭에 보여줄 페이지개수로 나눈 나머지값))
						//* 한블럭에 보여줄 페이지개수  + 1
						int startPage = ((currentPage / pageBlock) - (currentPage%pageBlock==0?1:0)) * pageBlock + 1;
						
						//끝페이지 번호 구하기
						//1 ~ 10 => 10    11 ~ 20 => 20    21 ~ 30 => 30
						//끝페이지 번호  = 시작페이지번호 + 한블럭에 보여줄 글개수
						int endPage = startPage + pageBlock - 1;
						
						//[이전] 시작페이지 번호가  한블럭에 보여줄 페이지 개수보다 클때~
