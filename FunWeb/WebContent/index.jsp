...
<header>
	...
	<%-- session영역에 값이 저장되어 있으면? 
		  로그인된 화면(logout<a>태그가 나오도록)으로 디자인 하고, 
		 session영역에 값이 저장되어 있지 않으면? 
		  로그인 되지 않는 화면(login <a>태그가 나오도록)으로 디자인하자.
	 --%>
	 <%
	 	String id = (String)session.getAttribute("id");
	 	
	 	if(id == null){ //세션값이 저장되지 않는 경우 
	 %>
	 	<div id="login">
	 ...
	 <%	
	 	}else{//세션영역에 세션값이 저장되어 있는 경우 로그인된 화면으로 처리 
	 %>	
	 	<div id="login">
	 		<%=id %>님 로그인중...
