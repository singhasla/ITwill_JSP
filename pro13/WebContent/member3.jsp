...
<body>
	...
	<%--
		...
		현재 member3.jsp페이지 내에서만 MemberBean객체를 공유해서 사용할 수 있다.
	 --%>
	...
	<%-- 
		memberForm.html(회원가입창)에서 전송된 요청값들을 request영역에서 얻어
		MemberBean객체의 setter메소드를 호출시 매개변수로 전달하여 각 변수에 저장
		
		<jsp:setProperty name="생성한 객체의 id값 넣어주기" 
						property="id" 
						value="<%=request.getParameter("id") %>"/>
		=> set~(String ~)메소드 호출시... 매개변수로 value속성값의 값을 전달하여 id인스턴스변수에 저장
	 --%>
