...
<body>
	...
	<%-- fn:substringing(string, StartIndexNum(0부터시작), EndIndexNum)함수는 string변수에 저장된 문자열의 일부분을 추출해 반환 --%>
	fn:substringing("Hello World!")를 호출한 결과 = ${fn:substring(title1, 0, 1)}<br>	<%-- H --%>
	
	<%-- fn:trim(string)함수는 string변수에 저장된 문자열의 앞뒤 공백을 제거해서 반환 --%>
	fn:trim(" abcd ")를 호출한 결과 = ${fn:trim(t1)}<br>	<%-- abcd --%>
	...
	fn:indexOf(title2, str1)를 호출한 결과 = ${fn:indexOf(title2, str1)}<br>	<%-- 4 --%>
	
	<%-- fn:contains(title2, str1)함수는 
		title2변수에 저장된 전체 문자열 중에서 str1변수에 저장된 문자열이 포함되어 있는지 판단하여 반환 --%>
	fn:contains(title2, str1)를 호출한 결과 = ${fn:contains(title2, str1)}<br>	<%-- true --%>
