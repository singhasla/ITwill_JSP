...
<body>
	...
	//1. 현재 실행중인 웹프로젝트에 대한 정보를 지니고 있는 
		...
		//multi.jsp페이지에서 입력한  이름, 주소, 하고싶은말 의 정보를 
		//request내장객체영역에서 꺼내오기 위해  
		//MultiPartRequest객체의 getParameter()메소드호출
		...
		//클라이언트가 업로드한 파일의 원본 이름을 저장할  가변길이 배열인? ArrayList객체 생성
		ArrayList originFiles = new ArrayList();
		
		
		//실제 업로드하기 위해 파일을 선택 하기 위한 <input>태그의 name속성값을 모두!~
		//Enmeration반복기역할을 하는 배열에 담아  Enmeration반복기 자체를 반환
		Enumeration e = multi.getFileNames();
		
		while(e.hasMoreElements()){
			
			//<input type="file">태그들의  name속성값들을 차례대로
			//Enumeration반복기역할을 하는 배열에서 꺼내온다
			Object object = e.nextElement();
			String filename = (String)object; 
			
			//웹서버 하드 공간에 실제로 업로드된 파일의 이름을 하나씩 얻어 
			//ArrayList배열에 추가하여 담기
			saveFiles.add(multi.getFilesystemName(filename));
			
			//클라이언트가 업로드한 파일의 원본이름 을 하나씩 얻어 
			//ArrayList배열에 추가하여 담기
	...
	<ul>
		...
		%>
			...
			<li>
				<%--파일다운로드를 처리하는 download.jsp페이지로 
				      ...
				      실제 업로드된 파일 명을 전달 하여  다운로드 처리 요청을 합니다.--%>
		...
		%>
	</ul>         http://localhost:8080/FileUpload/
