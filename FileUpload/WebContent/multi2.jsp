...
<body>
	...
	<form action="multi_pro.jsp" method="post" enctype="multipart/form-data" name="f" >
		...
		하고싶은말 : <br> <textarea rows="3" cols="70" name="note"></textarea> <br>
	...
	<script type="text/javascript">
		...
		function addInput(){
			...
			//입력한 <input>태그의 개수만큼 반복해서  
			//<input type="file">태그를 생성하여 div태그영역에 추가
			for(i=0;  i<filecnt; i++){
				msg += "<input type='file' name='upfile" + i + "'/><br>";
			}
			//div태그영역 내부에  msg변수에 저장된 <input>태그를 동적으로 추가 
			div.innerHTML = msg;
		}
	
	
		//2번 form태그에서 다중파일업로드요청 버튼 클릭했을때 호출되는 함수로 
		//   동적으로 만들어지는 <input type="file">태그중.. 누락된 곳이 있다면 검사해주는 곳
		function Check(f){
			...
			/*
			(설명) 
			  2번 form태그에 저희는 업로드될 파일개수를 사용자에 의해서 동적으로 입력받기 때문에
			  2번 form내부의 for문을 이용하여  <input type="file">태그가 동적으로 만들어집니다.
			   동적으로 만들어진 <input type="file">태그중에서 
			   선택하지 않은 <input>태그가 존재 한다면 
			  x번째 파일 정보를 선택 하지 않았습니다. <--- 경고메세지창을 나타내자
			*/
			...
			for(i=0; i<cnt; i++){		
				...
				if(f.elements[i].type == "file"){
					//만약 <input type="file">태그중에 선택하지 않은 태그가 있다면
					//(업로드할 파일을 지정하지 않았다면)
					if(f.elements[i].value == ""){
						//x번째? <input type="file">태그 내용이 비었습니다. 경고메세지!
						var msg = filecnt + "번째 파일정보가 누락되었습니다.\n모두다 선택해주세요";
						alert(msg);
						//지정하지 않은 <input type="file">인 태그에 포커스 주기
						f.elements[i].focus();
						//for문 빠져나가기
						return;
					}//안쪽 if 끝
					//<input>태그의 type속성의 값이 file일때만 filecnt변수의 값을 1증가
					filecnt++;
				}//바깥쪾 if 끝
			}//for 끝
