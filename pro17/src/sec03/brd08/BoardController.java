...
/*
     /listArticles.do 로 최초 모든글을 조회 하는 요청주소를 받았을때..
     section값과 pageNum의 기본값을 1로 초기화합니다.
        컨트롤러에서 전달된 section과 pageNum을 HashMap에 저장한후 DAO로 전달합니다.
...
//     /addReply.do얻기 
		...
		if(action == null){//요청한 주소가 없을때..
			...
			//section값과 pageNum값을 HashMap에 저장한후  BoardService객체의 메소드 호출시 전달
			Map pagingMap = new HashMap();
			pagingMap.put("section", section);
			pagingMap.put("pageNum", pageNum);
							
			//section값과 pageNum값으로 해당 섹션과 페이지에 해당되는 글목록을 조회하기 위해 호출!
			//조회한 글목록을 맵에 담아 반환 받습니다.
			...
			//글목록 화면에서  명시적으로 페이지 번호를 눌러서 요청한 경우  section값과 pageNum값을 가져옵니다.
			String _section = request.getParameter("section");
			String _pageNum = request.getParameter("pageNum");
			int section = Integer.parseInt( ( (_section == null) ? "1" : _section ) );       
			int pageNum = Integer.parseInt( ( (_pageNum == null) ? "1" : _pageNum ) );
