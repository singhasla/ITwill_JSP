...
public class BoardDAO {
	...
	//매개변수로 전달 받은 HashMap(section과 pageNum)을 이용해 SQL문으로 조회합니다.
	public List selectAllArticles(Map  pagingMap){
		
		List articleList = new ArrayList();
		
		//매개변수로 전달받은  HashMap내부의 section과 pageNum을 꺼내옵니다.
		...
		return articleList;//조회한 글들(ArticleVO객체들)이 저장되어 있는 ArrayList배열을  BoardService로 리턴 
	...
	public int selectTotArticles(){
		...
		return 0; //글수가 조회 되지 않을때  0을 리턴
	}
	
	
	
	/*삭제할 글에 대한 글번호를 검색해서 가져오는 메소드 */
	...
	public List<Integer> selectRemoveArticles(int articleNO){
		...
		return articleNOList;//조회된 모들 글번호들이 저장된 ArrayList배열을  BoardService로 반환
	}//selectRemoveArticles메소드 끝
	
	
	//매개변수로 잔달 받은 삭제할 부모 글번호에 해당하는 자식글 까지 모두 삭제 시키는 메소드 
	public void deleteArticle(int articleNO){		
		/*
		참고
		오라클의 계층형 SQL문을 이용해 부모글에 대한 자식글을 모두 삭제 하는 SQL문입니다.
		DELETE FROM t_board
		WHERE articleNO in(  
		...
		글번호가 2인 글과  그 자식(답변글)글 들을 모두 삭제하라는 의미 입니다.				  
		*/
		try{
			//커넥션풀로 부터 커넥션 얻기 (DB연결)
			conn = dataFactory.getConnection();
			
			//매개변수로 전달 받는  부모글번호에 해당하는 글과 그에 연결된 모든 답변글 까지 삭제 시키는 SQL문 만들기
