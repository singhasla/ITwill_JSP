 ...
 *  /viewArticle.do로 요청하여 글 번호를 받아온다.
 *  그리고 그 번호에 해당하는 글정보를 DB에서 조회한 후 가져와 request메모리에 바인딩한 후
 *  viewArticle.jsp뷰페이지로 전송함(viewArticles.jsp페이지 아님)
 */
...
//		
		...
		//글제목을 클릭해 글내용 조회요청을 받았을때...
			...
			//글 번호에 해당되는 글 정보를 조회하기 위해 BoardService객체의 viewArticle()메소드 호출시
			//글번호를 전달하여 조회해 옴
			articleVO = boardService.viewArticle(Integer.parseInt(articleNO));
			
			//조회한 글정보(articleVO객체)를 View페이지(/board03/viewArticle.jsp)로 보내주기 위해
			//request내장객체영역에 조회한 글정보(articleVO객체)를 저장(바인딩)
